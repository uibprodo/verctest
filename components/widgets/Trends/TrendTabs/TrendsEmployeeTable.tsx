import data from '@/mocks/employeeTrendsMock.json';
import { TrendsTableStructure } from '@/utils/types';

type Props = {
  tableStructure: TrendsTableStructure[];
};
const TrendsEmployeeTable: React.FC<Props> = ({ tableStructure }) => {
  return (
    <>
      {data?.emp_prod_data?.map((row, index) => (
        <tr
          key={`row-${index}`}
...
export default TrendsEmployeeTable;
